...
//                (K==='Class' && k==='ErrorClass') || // Type.isClass(ErrorClass: function Error() { [native code] })
    ...
    a.f(Type.isObject(null))
    a.f(Type.isObject(undefined))
    a.f(Type.isObject(false))
    a.f(Type.isObject(0))
    a.f(Type.isObject(0n))
    a.f(Type.isObject(0.0))
    a.f(Type.isObject(''))
    a.f(Type.isObject(Symbol()))
    ...
    ;[null,NaN,Infinity,true,false,0,1,-1,0n,'A',Symbol(),new Proxy({},{})].map(v=>a.f(Type.isUndefined(v)));
    ;[new Boolean(),new Number(),new String(),new Function(),new Proxy({},{}),new Promise(()=>{})].map(v=>a.f(Type.isUndefined(v)));

    a.t(Type.isNull(null))
    ;[undefined,NaN,Infinity,true,false,0,1,-1,0n,'A',Symbol(),new Proxy({},{})].map(v=>a.f(Type.isNull(v)));
...
//    Type.to('class', )
    ...
    */
    ...
    a.t(Type.isIns(Type.name))
    a.t(Type.isFn(Type.name.get))
    a.t(Type.isFn(Type.name.valid))
    a.t(Type.isFn(Type.name.is))
    // valid()
    ;(function(){
        for (let [k,v] of Type._names) {
            a.t(Type.name.valid(k))
            const [abbrs,fn] = v
            a.t(abbrs.every(abbr=>Type.name.valid(abbr)))
        }
        a.t(Type.name.valid('C'))
    });
    a.t('Integer'===Type.name.get('int'))
    a.t('String'===Type.name.get('str'))
    a.t('Integer'===Type.name.get('inteGER'))

    a.t(Type.name.is('int', 0))
    a.f(Type.name.is('int', 'A'))
    a.e(TypeError, `'x'は不正な型名です。`, ()=>Type.name.is('x', 'A'))
    /*
    ;(function(){
        for (let [k,v] of Type._names) {
            a.t(Type.isValid(k))
            const [abbrs,fn] = v
            a.t(abbrs.every(abbr=>Type.isValid(abbr)))
        }
        a.t(Type.isValid('C'))
    });
    */
...
//        a.t(ins2.constructor.constructor.hasOwnProperty('sm'))
    ...
    ;(function(){
        ...
        console.log(Type.NOT_EXIST_FIELD===Type.getField(obj, 'fn'))
        //a.t(undefined===Type.getField(obj, 'fn')) // Fieldは値がundefinedの場合もある。キーが未存在の場合と区別する必要あり
    ...
    ;(function(){
        ...
        console.log(Type.get(ins, 'a'))
        console.log(Type.getMember(ins, 'a'))
        a.t(0===Type.get(ins, 'a'))
        a.t(0===Type.getMember(ins, 'a'))
        ...
        console.log(Object.getOwnPropertyNames({a:0,fn:()=>{}}))
        console.log(Object.getOwnPropertyNames(ins))
//        const G = Object.getOwnPropertyDescriptor(ins, 'g')
        //const G = Object.getOwnPropertyDescriptor(D, 'g')
        //const G = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(D), 'g')
        //const G = Object.getOwnPropertyDescriptor(C, 'g')
        //const G = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(C), 'g')
        //const G = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(ins), 'g')
        const G = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(Object.getPrototypeOf(ins)), 'g')
        console.log(G)
        console.log(C, D, ins)
        ...
        a.t(undefined===Type.getFn(ins, 'g'))
        //a.t(G.get===Type.getMethod(ins, 'g'))
        a.t(undefined===Type.getMethod(ins, 'g'))
        a.t(undefined===Type.getStaticMethod(ins, 'g'))
        a.t(G.get===Type.getGetter(ins, 'g'))
        a.t(undefined===Type.getSetter(ins, 'g'))
