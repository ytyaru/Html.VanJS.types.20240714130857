...
class Type {
    constructor() {
        ...
        this._names.set('Boolean', ['Bool', 'Bln', 'B'])
        this._names.set('NaN', [])
        ...
        this._names.set('Primitive', ['Prim'])
        this._names.set('ValidPrimitive', ['Vprim', 'Vp']) // null,undefinedを抜いたprimitive
        this._names.set('Array', ['Ary', 'A'])
        this._names.set('Object', ['Obj', 'O'])
        this._names.set('Promise', ['Prms', 'Prm'])
        this._names.set('Iterator', ['Iter', 'Itr', 'It'])
        this._names.set('Function', ['Func', 'Fn'])
        this._names.set('AsyncFunction', ['AsyncFunc', 'AsyncFn', 'Afn'])
        this._names.set('GeneratorFunction', ['GenFn', 'Gfn'])
        this._names.set('AsyncGeneratorFunction', ['AsyncGenFn', 'Agfn'])
        ...
        for (let [k,v] of this._names.entries()) {
            ...
            if ('function'!==getter) { throw new Error(`${fnName}が未定義です。`)}
            for (let name of v) { this.#defineAbbr(`is${name}`, getter) }
        }
    }
...
//    #isUndefined(v) { return undefined===v }
    isNullOrUndefined(v) { return this.isNull(v) || this.isUndefined(v) }
...
//    #isNU(v) { return this.isNull(v) || this.#isUndefined(v) }
    ...
    isNaN(v) { return Number.isNaN(v) } // 定数NaNと完全一致するか判定する唯一の方法 https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Number/isNaN

    //isPrimitive(v) { return (!this.isNU(v) && !this.isObj(v) !this.isFn(v)) }
    isPrimitive(v) { return (!this.isNU(v) && !this.isFn(v) && 'object'!==typeof v) }

    isAsyncFunction(v) { return v instanceof (async()=>{}).constructor }
...
//        return true
    ...
    isInstance(i, c) { // i:instance, c:ClassType
        ...
        return this.isClass(c) ? c===i.constructor : true
    }
//    #call(name, v) {
